...
tmat <- cqnnorm(xd,ABRFABgcl[,1],ABRFABgcl[,2])
...
write.table(cbind(zscore,zp),"ABRFAB.txt",quote=F,sep="\t",row.names=F,col.names = F)

aucs <- c(qtroc$auc,tmroc$auc,troc$auc,uproc$auc,dsroc$auc,cqroc$auc,medroc$auc,uqroc$auc)

write.table(aucs,"ABRFAB_AUC.txt",quote=F,sep="\t",row.names=F,col.names = F)
##plot
pdf("ABRFAUC_norm.pdf",width=6,height=6)
...
lines(c(0.65,0.7),rep(0.52,2),lwd=3,col=2);#text(0.80,0.3,"qtotal");
lines(c(0.65,0.7),rep(0.45,2),lwd=3,col=3);#text(0.79,0.25,"TMM");
lines(c(0.65,0.7),rep(0.38,2),lwd=3,col=4);#text(0.79,0.2,"Total");
lines(c(0.65,0.7),rep(0.31,2),lwd=3,col=5);#text(0.80,0.15,"quartile");
lines(c(0.65,0.7),rep(0.24,2),lwd=3,col=6);#text(0.79,0.10,"Geometric");
lines(c(0.65,0.7),rep(0.17,2),lwd=3,col=7);#text(0.79,0.05,"cqn");
lines(c(0.65,0.7),rep(0.10,2),lwd=3,col=8);#text(0.79,0.35,"MedpgQ2");
lines(c(0.65,0.7),rep(0.03,2),lwd=3,col=1);#text(0.79,0.35,"UppgQ2");
dev.off()


pdf("ABRFAUC_ERCC.pdf",width=6,height=6)

par(mar=c(0,0,0,0)+5)
#
dat <- c(qtroc$erccRM,tmroc$erccRM,troc$erccRM,uproc$erccRM,
         dsroc$erccRM,cqroc$erccRM,medroc$erccRM,uqroc$erccRM)
barplot(dat,main="",xlab="",ylab="",col=c(2:8,1),axes=F,xlim=c(0,9.5),ylim=c(0,1))
axis(1,at=seq(0.7,9.3,1.2),labels=FALSE)
axis(2,at=seq(0,1,length=5),labels=FALSE)

dev.off()
pdf("ABRFAUC_RM.pdf",width=6,height=6)

par(mar=c(0,0,0,0)+5)
#
dat <- c(qtroc$RM,tmroc$RM,troc$RM,uproc$RM,
         dsroc$RM,cqroc$RM,medroc$RM,uqroc$RM)
barplot(dat,main="",xlab="",ylab="",col=c(2:8,1),axes=F,xlim=c(0,9.5),ylim=c(0,8))
axis(1,at=seq(0.7,9.3,1.2),labels=FALSE)
