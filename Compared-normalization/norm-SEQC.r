...
tmat <- cqnnorm(xd,SEQCgcl[,1],SEQCgcl[,2])
...
write.table(cbind(zscore,zp),"SEQC.txt",quote=F,sep="\t",row.names=F,col.names = F)

aucs <- c(qtroc$auc,tmroc$auc,troc$auc,uproc$auc,dsroc$auc,cqroc$auc,medroc$auc,uqroc$auc)

write.table(aucs,"SEQC_AUC.txt",quote=F,sep="\t",row.names=F,col.names = F)

##plot
pdf("SEQCAUC_norm.pdf",width=6,height=6)
...
pdf("SEQCAUC_ERCC.pdf",width=6,height=6)

par(mar=c(0,0,0,0)+5)
#
dat <- c(qtroc$erccRM,tmroc$erccRM,troc$erccRM,uproc$erccRM,
         dsroc$erccRM,cqroc$erccRM,medroc$erccRM,uqroc$erccRM)
barplot(dat,main="",xlab="",ylab="",col=c(2:8,1),axes=F,xlim=c(0,9.5),ylim=c(0,1.25))
axis(1,at=seq(0.7,9.3,1.2),labels=FALSE)
axis(2,at=seq(0,1.25,length=6),labels=FALSE)

dev.off()
pdf("SEQCAUC_RM.pdf",width=6,height=6)
