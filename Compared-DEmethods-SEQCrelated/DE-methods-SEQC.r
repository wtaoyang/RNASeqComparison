...
#gname gene name
qpcID <- rownames(PrimePCR_gn)
#Tp
pset <- qpcID[abs(PrimePCR_gn[,1])>0.5]
nset <- qpcID[abs(PrimePCR_gn[,1])<0.2]
##logCPM
library(edgeR)
...
condA <- 1:5
condB <- 6:10
cond <- factor(c(rep("condA",5),rep("condB",5)))
...
statsCal <- function(tfc,pv,adpv)
{
  ...
  ind <- tfc[pset]*PrimePCR_gn[pset,1] <=0
  npset <- pset[!ind]
  nnset <- nset
  roc<- roccurve(gname[ord],npset,nnset)
  roc$erccRM <- RMSD(tfc[erccid],-ERCCinfo[erccid,5])
  roc$RM <- RMSD(tfc[gnamefilter],PrimePCR_gn[gnamefilter,1])
  np <- length(npset)
  nf <- length(nnset)
  roc$np <- np
  roc$nf <- nf
  ...
  x <- sum(aset%in%npset)#/np
  y <- sum(aset%in%nnset)#/(#fncurve(gname[ord],pset,nset)
  roc$efdr <- y/(x+y)
  roc$sens <- x/np
  return(roc)
}
...
##aFold
tafold <- read.table("tafold-seqc.txt",header=T,row.names=1,sep="\t")#ResaFold(xd,cond)
tfc <- tafold[,1]
names(tfc) <- gname
aroc <- statsCal(tfc,adpv=tafold[,3])
write.table(tafold,"tafold-seqc.txt",quote=F,sep="\t")
...
write.table(tafoldtmm,"tafold-tmm-seqc.txt",quote=F,sep="\t")
...
write.table(tafoldgeo,"tafold-geo-seqc.txt",quote=F,sep="\t")
...
write.table(tafoldqua,"tafold-qua-seqc.txt",quote=F,sep="\t")

##DESEQ2
tdeseq2 <- read.table("tdeseq2-seqc.txt",header=T,row.names=1,sep="\t")#ResDESeq2(xd,cond)
tfc <- tdeseq2[,1]
pv <- tdeseq2[,2]
names(tfc) <- gname
dsroc <- statsCal(tfc,pv,adpv=tdeseq2[,3])
write.table(tdeseq2,"tdeseq2-seqc.txt",quote=F,sep="\t")


##DESEQ2 qtotal
tdeseq2q <- read.table("tdeseq2-q-seqc.txt",header=T,row.names=1,sep="\t")#ResDESeq2(xd,cond,sf=sfq)
tfc <- tdeseq2q[,1]
pv <- tdeseq2q[,2]
names(tfc) <- gname
dsrocq <- statsCal(tfc,pv,adpv=tdeseq2q[,3])
write.table(tdeseq2q,"tdeseq2-q-seqc.txt",quote=F,sep="\t")
##edgeR
tedgeR <- read.table("tedgeR-seqc.txt",header=T,row.names=1,sep="\t")#ResedgeR(xd,cond)
tedgeR <- tedgeR[rownames(xd),]
tfc <- tedgeR[,1]
pv <- tedgeR[,2]
names(tfc) <- gname
edroc <- statsCal(tfc,pv,adpv=tedgeR[,3])
write.table(tedgeR,"tedgeR-seqc.txt",quote=F,sep="\t")
##edgeR qtotal
tedgeRq <- read.table("tedgeR-q-seqc.txt",header=T,row.names=1,sep="\t")#ResedgeR(xd,cond,sf=sfq)
tedgeRq <- tedgeRq[rownames(xd),]
tfc <- tedgeRq[,1]
pv <- tedgeRq[,2]
names(tfc) <- gname
edrocq <- statsCal(tfc,pv,adpv=tedgeRq[,3])
write.table(tedgeRq,"tedgeR-q-seqc.txt",quote=F,sep="\t")
##voom

tVoom <- read.table("tVoom-seqc.txt",header=T,row.names=1,sep="\t")#ResVoom(xd,cond)
tVoom <- tVoom[rownames(xd),]
tfc <- tVoom[,1]
names(tfc) <- gname
pv <- tVoom[,2]
vmroc <- statsCal(tfc,pv,adpv=tVoom[,3])
write.table(tVoom,"tVoom-seqc.txt",quote=F,sep="\t")

##voom qtotal

tVoomq <- read.table("tVoom-q-seqc.txt",header=T,row.names=1,sep="\t")#ResVoom(xd,cond,sf=sfq)
tVoomq <- tVoomq[rownames(xd),]
tfc <- tVoomq[,1]
names(tfc) <- gname
pv <- tVoomq[,2]
vmrocq <- statsCal(tfc,pv,adpv=tVoomq[,3])
write.table(tVoomq,"tVoom-q-seqc.txt",quote=F,sep="\t")


##baySeq
#tbaySeq <- ResbaySeq(SEQC,cond,cl=8)
tbaySeq <- read.table("SEQC_res.txt",header=T,row.names=1,sep="\t")
...
##baySeq qtotal
#tbaySeqq <- ResbaySeq(SEQC,cond,cl=8)
tbaySeqq <- read.table("SEQC_res_qt.txt",header=T,row.names=1,sep="\t")
...
tABSSeq <- read.table("tABSSeq-seqc.txt",header=T,row.names=1,sep="\t")#ResABSSeq(xd,cond)
tfc <- tABSSeq[,1]
names(tfc) <- gname
pv <- tABSSeq[,2]
abroc <- statsCal(tfc,pv,adpv=tABSSeq[,3])
write.table(tABSSeq,"tABSSeq-seqc.txt",quote=F,sep="\t")

##ROTS-total
source("./Rscript/NormMethods.r")
x <- log2(totalnorm(xd)+1)
tROTSt <- read.table("tROTSt-seqc.txt",header=T,row.names=1,sep="\t")#ResROTS(x,cond)
tROTSt <- tROTSt[rownames(xd),]
tfc <- -tROTSt[,1]
names(tfc) <- gname
pv <- tROTSt[,2]
rotroc <- statsCal(tfc,pv,adpv=tROTSt[,3])
#write.table(tROTSt,"tROTSt-seqc.txt",quote=F,sep="\t")
##ROTS-qtotal
x <- log2(qtotalnorm(xd)+1)
qROTSt <- read.table("qROTSt-seqc.txt",header=T,row.names=1,sep="\t")#ResROTS(x,cond)
...
write.table(qROTSt,"qROTSt-seqc.txt",quote=F,sep="\t")
...
write.table(cbind(zscore,zp),"SEQC_DE_all.txt",quote=F,sep="\t",row.names=F,col.names = F)
...
write.table(aucs,"SEQC_DE_all_AUC.txt",quote=F,sep="\t",row.names=F,col.names = F)
...
zp <- pnorm(zscore,lower.tail = F)*length(zscore)
write.table(cbind(zscore,zp),"SEQC_DE_afold_switch.txt",quote=F,sep="\t",row.names=F,col.names = F)


aucs <- c(aroct$auc,arocq$auc,arocg$auc)

write.table(aucs,"SEQC_DE_all_AUC_afold_swtich.txt",quote=F,sep="\t",row.names=F,col.names = F)

pdf("SEQCAUC_de.pdf",width=6,height=6)
...
lines(seq(0,1,0.05),seq(0,1,0.05),lwd=3,col="black",lty=2)
...
pdf("SEQCAUC_de_afold.pdf",width=6,height=6)
...
pdf("SEQCAUC_ERCC_de.pdf",width=6,height=6)
...
dev.off()
pdf("SEQCAUC_RM_de.pdf",width=6,height=6)
...
pdf("SEQCAUC_RM_de_all.pdf",width=8,height=6)
...
pdf("SEQCAUC_ercc_de_all.pdf",width=8,height=6)
...
dev.off()
##get plot for FC from PrimePCR and SEQC
lcp <- calCPM(xd,cond)
names(lcp) <- gname
lcp <- lcp[gnamefilter]
...
#cor(x[lcp>1],y[lcp>1])
pdf(file="SEQC-af-Prime.pdf",width=8,height=6)
x <-afd[gnamefilter]
y<-PrimePCR_gn[gnamefilter,1]
...
dev.off()
pdf(file="SEQC-raw-Prime.pdf",width=8,height=6)
x <-rwd[gnamefilter]
y<-PrimePCR_gn[gnamefilter,1]
...
pdf(file="SEQC-ds-Prime.pdf",width=8,height=6)
x <-dsd[gnamefilter]
y<-PrimePCR_gn[gnamefilter,1]
