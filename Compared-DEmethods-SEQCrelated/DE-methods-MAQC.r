...
##aFold
tafold <- read.table("tafold-MAQC.txt",header=T,row.names=1,sep="\t")#ResaFold(xd,cond)
tfc <- tafold[,1]
names(tfc) <- gname
aroc <- statsCal(tfc,adpv=tafold[,3])
write.table(tafold,"tafold-MAQC.txt",quote=F,sep="\t")
...
write.table(tafoldtmm,"tafold-tmm-MAQC.txt",quote=F,sep="\t")
...
write.table(tafoldgeo,"tafold-geo-MAQC.txt",quote=F,sep="\t")
...
write.table(tafoldqua,"tafold-qua-MAQC.txt",quote=F,sep="\t")

##DESEQ2
tdeseq2 <- read.table("tdeseq2-MAQC.txt",header=T,row.names=1,sep="\t")#ResDESeq2(xd,cond)
tfc <- tdeseq2[,1]
pv <- tdeseq2[,2]
names(tfc) <- gname
dsroc <- statsCal(tfc,pv,adpv=tdeseq2[,3])
write.table(tdeseq2,"tdeseq2-MAQC.txt",quote=F,sep="\t")
...
write.table(tdeseq2q,"tdeseq2-q-MAQC.txt",quote=F,sep="\t")
##edgeR
tedgeR <- read.table("tedgeR-MAQC.txt",header=T,row.names=1,sep="\t")#ResedgeR(xd,cond)
tedgeR <- tedgeR[rownames(xd),]
tfc <- tedgeR[,1]
pv <- tedgeR[,2]
names(tfc) <- gname
edroc <- statsCal(tfc,pv,adpv=tedgeR[,3])
write.table(tedgeR,"tedgeR-MAQC.txt",quote=F,sep="\t")
##edgeR qtotal
...
write.table(tedgeRq,"tedgeR-q-MAQC.txt",quote=F,sep="\t")
##voom

tVoom <- read.table("tVoom-MAQC.txt",header=T,row.names=1,sep="\t")#ResVoom(xd,cond)
tVoom <- tVoom[rownames(xd),]
tfc <- tVoom[,1]
names(tfc) <- gname
pv <- tVoom[,2]
vmroc <- statsCal(tfc,pv,adpv=tVoom[,3])
write.table(tVoom,"tVoom-MAQC.txt",quote=F,sep="\t")
...
write.table(tVoomq,"tVoom-q-MAQC.txt",quote=F,sep="\t")


##baySeq
#tbaySeq <- ResbaySeq(xd,cond,cl=8)
tbaySeq <- read.table("MAQC_res.txt",header=T,row.names=1,sep="\t")
...
tbaySeqq <- read.table("MAQC_res_qt.txt",header=T,row.names=1,sep="\t")
...
tABSSeq <- read.table("tABSSeq-MAQC.txt",header=T,row.names=1,sep="\t")#ResABSSeq(xd,cond)
tfc <- tABSSeq[,1]
names(tfc) <- gname
pv <- tABSSeq[,2]
abroc <- statsCal(tfc,pv,adpv=tABSSeq[,3])
write.table(tABSSeq,"tABSSeq-MAQC.txt",quote=F,sep="\t")

##ROTS-total
source("./Rscript/NormMethods.r")
x <- log2(totalnorm(xd)+1)
tROTSt <- read.table("tROTSt-MAQC.txt",header=T,row.names=1,sep="\t")#ResROTS(x,cond)
tROTSt <- tROTSt[rownames(xd),]
tfc <- -tROTSt[,1]
names(tfc) <- gname
pv <- tROTSt[,2]
rotroc <- statsCal(tfc,pv,adpv=tROTSt[,3])
write.table(tROTSt,"tROTSt-MAQC.txt",quote=F,sep="\t")
##ROTS-qtotal
x <- log2(qtotalnorm(xd)+1)
qROTSt <- read.table("qROTSt-MAQC.txt",header=T,row.names=1,sep="\t")#ResROTS(x,cond)
...
write.table(qROTSt,"qROTSt-MAQC.txt",quote=F,sep="\t")
...
write.table(cbind(zscore,zp),"MAQC_DE_all.txt",quote=F,sep="\t",row.names=F,col.names = F)
...
write.table(aucs,"MAQC_DE_all_AUC.txt",quote=F,sep="\t",row.names=F,col.names = F)
...
zp <- pnorm(zscore,lower.tail = F)*length(zscore)
write.table(cbind(zscore,zp),"MAQC_DE_afold_switch.txt",quote=F,sep="\t",row.names=F,col.names = F)


aucs <- c(aroct$auc,arocq$auc,arocg$auc)

write.table(aucs,"MAQC_DE_all_AUC_afold_swtich.txt",quote=F,sep="\t",row.names=F,col.names = F)
